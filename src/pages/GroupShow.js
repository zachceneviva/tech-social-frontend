...
export default function GroupShow () {
    ...
    useEffect(() =>{
        axios.get(`http://localhost:4000/api/v1/techonnect/groups/${params.id}`).then(res => {
    ...
    const handlePost = async (e) => {
        ...
        {content: postContent, image: `https://${postImage}`, github: `https://github.com/${postGh}`, link: `https://${postLink}`, user: user, group: params.id}).then( res => console.log(res))
        setPostContent('')
        setPostImage('')
        setPostGh('')
        setPostLink('')
